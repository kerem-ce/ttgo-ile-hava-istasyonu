...
#define TFT_GREY 0x07FF
#define lightblue 0xF800
#define darkred 0xF800
#define blue 0x001F
...
const char* ssid     = "Redmi";       //WİFİ ADI
const char* password = "06krm28SDA"; //WİFİ ŞİFRESİ
String town="Batikent";              //BULUNDUĞUNUZ İLÇE
String Country="TR";                //BULUNDUĞUNUZ ÜLKENİN KISALTIMI
const String endpoint = "http://api.openweathermap.org/data/2.5/weather?q="+town+","+Country+"&units=metric&APPID=";
const String key = "baa4a0ec485e04253892acbe7cb232bb"; /*WATERMAP TARAFINDAN MAİL ADRESİNİZE GÖNDERİLEN APİ KEY                   */
...
// Tarih ve saati kaydedecek değişkenler
...
void setup(void) {
  ...
  tft.println("");
  tft.println("WiFi BAGLANDI.");
  tft.println("IP ADRES: ");
...
// Zaman almak için NTPClient'i başlat
  timeClient.begin(); 
...
void getData()
{
   ...
   if ((WiFi.status() == WL_CONNECTED)) { //Mevcut bağlantı durumunu kontrol et
 
    HTTPClient http;
 
    http.begin(endpoint + key); //URL'yi belirtin
    int httpCode = http.GET();  //isteği yap
 
    if (httpCode > 0) { // Dönen kodu kontrol et
    ...
    http.end(); // Kaynakları serbest bırak
  }
 char inp[1000];
